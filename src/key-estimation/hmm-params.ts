import key_hmm_params_raw from "../assets/key_hmm_params_raw.json"
...
function normalizeLogProb(scores: number[], temperature: number, targetMinimum: number) {
  ...
  const scale = targetMinimum / Math.log(Math.exp(-maxScore / temperature) / sumExpScores)
...
const emissionProb = key_hmm_params_raw.emission_counts.map(
  ...
  (mode_counts, i) => normalizeLogProb(
    i === 0 ? softClamp(mode_counts.flat(), 0, 400, 0.01) : softClamp(mode_counts.flat(), 0, 400, 0.01),
    i === 0 ? 20 : 30,
    -50)
)
...
export function parseKey(key: string): [key: number, isMinor: boolean] {
  if (key.includes(":")) {
    ...
    return [PITCH_TO_INT[tonic.trim() as keyof typeof PITCH_TO_INT] ?? -1, isMinor]
  } else {
...
export function debugChordNaming(chordName: string | null | undefined, key: string) {
...
export function chordTransitionLogProb(curKey: string, lastChord: string | undefined | null, nextChord: string): number {

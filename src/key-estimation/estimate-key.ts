...
function decodeHmm(chords: ChordSegment[]) {
  ...
  const averageDuration = chords.reduce((acc, cur) => acc + cur.end - cur.start, 0) / length
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    for (let currentKeyIndex = 0; currentKeyIndex < 24; currentKeyIndex += 1) {
      ...
      const keyTransitions = empty.map((_, key) =>
        lastKeyLogProbs[key] + checkProb(keyTransitionLogProb(scaleCandidates[key], currentKey), i, currentKey)
      )
      const lastKey = argMax(keyTransitions)

      // emission scores
      const chordTransition = checkProb(chordTransitionLogProb(currentKey, lastChord, chord), i, currentKey, lastChord, chord)

      // if chord duration is very long, repeat chord transition is more likely
      const repeatChord = duration < averageDuration * 1.7 ? 0 : checkProb(chordTransitionLogProb(currentKey, chord, chord), i, currentKey, chord)
